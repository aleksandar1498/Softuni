...
public class MissionManagerImpl implements MissionManager {
    ...
    public String request(List<String> arguments) {
        ...
        return String.format("Assigned %s Mission - %s to Agent - %s", mission.getClass().getSimpleName(), missionId, this.agents.get(agentId).getName());
    }
    ...
    public String status(List<String> arguments) {
        ...
        if(this.agents.containsKey(id)){
            ...
            status = String.format("Assigned Mission – %s", assignedMission.getId()) +
            ...
            status = String.format("Completed Mission – %s", completedMission.getId()) +
                    System.lineSeparator() +
                    "Status: Closed" +
                    System.lineSeparator() +
    ...
    public String over(List<String> arguments) {
        ...
        long totAssigned = this.agents.values().stream().flatMap(a -> {
            Map<String, Mission> assigned = null;
            try {
                assigned = (LinkedHashMap<String, Mission>) a.getClass().getDeclaredField("assignedMissions").get(a);
        ...
        long totCompleted = this.agents.values().stream().flatMap(a -> {
            Map<String, Mission> completed = null;
            try {
                completed = (LinkedHashMap<String, Mission>) a.getClass().getDeclaredField("completedMissions").get(a);
        ...
        Double totalRatingGiven = this.agents.values().stream().mapToDouble(a -> {
            Map<String, Mission> completed = null;
            Map<String, Mission> assigned = null;
            try {
                completed = (LinkedHashMap<String, Mission>) a.getClass().getDeclaredField("completedMissions").get(a);
            ...
            return completed.values().stream().mapToDouble(Rateable::getRating).sum() + assigned.values().stream().mapToDouble(Rateable::getRating).sum();
        }).sum();
        Double totalBountyGiven = this.agents.values().stream().mapToDouble(a -> {
            ...
            return completed.values().stream().mapToDouble(Bountyable::getBounty).sum() ;
        }).sum();
